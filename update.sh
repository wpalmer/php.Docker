...
for version in "${versions[@]}"; do
	...
	if [ -f "$version-Dockerfile-block-1" ]; then
		echo "Generating $version/Dockerfile from $base + $version-Dockerfile-block-*"
		{
			...
			awk '
				...
				ia { ac++ }
				ia && ac == 1 {
					if(system("[ -f '$version'-Dockerfile-block-" ab " ]") == 0) {
						system("cat '$version'-Dockerfile-block-" ab)
					}
				}
	...
	; do
		...
		if \
			...
			[ -d "$version/${target%%-bundle-*}" ]
		then
			# this is a bundle based on something else. Copy extra files.
			find "$version/${target%%-bundle-*}" \
				-type f \
				-not -name 'docker-php-ext-*' \
				-not -name 'docker-php-source' \
				-not -name 'Dockerfile' \
				-exec cp -a -v '{}' "$version/$target/" ';'
		fi
